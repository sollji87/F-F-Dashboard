...
/**
 * Snowflake 데이터에서 매장 수 가져오기
 * CHNL_CD NOT IN ('0','8','9','99') 조건에 해당하는 채널만 합산
 */
export function getStoreCountFromSnowflake(snowflakeData, month, brandCode) {
  if (!snowflakeData || !snowflakeData.stores) {
    return null;
  }
  
  const brandStores = snowflakeData.stores.filter(s => 
    BRAND_CODE_MAP[s.brand_code] === brandCode && s.month === month
  );
  
  const totalCount = brandStores.reduce((sum, s) => sum + s.store_count, 0);
  console.log(`📍 매장수 [${brandCode} ${month}]:`, totalCount, '개');
  return totalCount;
}

/**
 * CSV 파일에서 매장 수 데이터 읽기 (서버 측) - Fallback용
...
export async function loadSalesFromCSV() {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const totalSales = parseFloat(values[3]?.trim().replace(/['"]/g, '').replace(/,/g, '')) || 0;
...
export async function loadCostsFromCSV() {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const month = values[0]?.trim().replace(/['"]/g, '');
      const brandCode = values[1]?.trim().replace(/['"]/g, '');
      const costAmt = parseFloat(values[11]?.trim().replace(/['"]/g, '').replace(/,/g, '')) || 0;
...
/**
 * Snowflake CSV 파일에서 매장수 데이터 읽기 (서버 측)
 */
export async function loadStoresFromCSV() {
  try {
    const filePath = path.join(process.cwd(), 'public', 'data', 'snowflake_stores.csv');
    
    if (!fs.existsSync(filePath)) {
      console.warn('매장수 CSV 파일을 찾을 수 없습니다:', filePath);
      return null;
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const month = values[0]?.trim().replace(/['"]/g, '');
      const brandCode = values[1]?.trim().replace(/['"]/g, '');
      const storeCount = parseInt(values[5]?.trim().replace(/['"]/g, '').replace(/,/g, '')) || 0;
      
      storesData.push({
        month: month,
        brand_code: brandCode,
        brand_name: values[2]?.trim().replace(/['"]/g, ''),
        channel_code: values[3]?.trim().replace(/['"]/g, ''),
        channel_name: values[4]?.trim().replace(/['"]/g, ''),
        store_count: storeCount,
      });
    }
    
    console.log('✅ 매장수 데이터 로드:', storesData.length, '건');
    
    return storesData;
  } catch (error) {
    console.error('매장수 CSV 로딩 에러:', error);
    return null;
  }
}
...
export async function loadSnowflakeData(month) {
  ...
  const storesData = await loadStoresFromCSV();
...
export async function loadBrandData(brandCode, currentMonth = '202510') {
  ...
  // 매장 수 로드 (Snowflake 우선, CSV는 fallback)
  let storeCount = getStoreCountFromSnowflake(snowflakeData, currentMonth, brandCode);
  if (!storeCount) {
    const storeCountData = await loadStoreCountFromCSV(currentMonth, brandCode);
    storeCount = storeCountData || 0;
  }
  
  // 전년 매장 수 로드 (Snowflake 우선, CSV는 fallback)
  let prevStoreCount = getStoreCountFromSnowflake(snowflakeData, previousYearMonth, brandCode);
  if (!prevStoreCount) {
    const prevStoreCountData = await loadStoreCountFromCSV(previousYearMonth, brandCode);
    prevStoreCount = prevStoreCountData || storeCount; // fallback to current store count
  }
  ...
  for (let m = 1; m <= monthNum; m++) {
    ...
    // 해당 월의 매장 수 로드 (Snowflake 우선)
    let monthStoreCount = getStoreCountFromSnowflake(snowflakeData, monthStr, brandCode);
    if (!monthStoreCount) {
      monthStoreCount = await loadStoreCountFromCSV(monthStr, brandCode);
    }
    ...
    if (snowflakeData && snowflakeData.costs && snowflakeData.sales) {
      ...
      // 급료와 임금 총액 계산
      const monthSalary = monthCosts
        .filter(c => c.category_l3 === '급료와 임금')
        .reduce((sum, c) => sum + c.cost_amt, 0);
      ...
      Object.entries(categoryMap).forEach(([category, cost]) => {
        monthly_data.push({
          ...
          salary_amt: monthSalary, // 급료와 임금 총액
