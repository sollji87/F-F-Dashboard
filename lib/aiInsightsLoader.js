...
export function loadCategoryInsightsFromCSV(brandCode, category, month) {
  try {
    const csvPath = path.join(process.cwd(), 'public', 'data', 'category_insights', `${brandCode}_${category}_${month}.csv`);
...
export function saveCategoryInsightsToCSV(brandCode, category, month, insights) {
  try {
    ...
    const csvPath = path.join(categoryInsightsDir, `${brandCode}_${category}_${month}.csv`);
